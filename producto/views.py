...
from produccion.models import  ProduccionRealizada,ProduccionEsperadaCategoria
from produccion.forms import  produccionECForm
...
def ver_categoria(request, categoria_slug):
    ...
    prodescat = ProduccionEsperadaCategoria.objects.all()
    if request.POST:
        form = produccionECForm(request.POST)
        if form.is_valid():
            prodescat = form.save(commit = False)
            prodescat.save()
            return redirect('catalogo')
    else:
        form = produccionECForm()
    args = {}
    args.update(csrf(request))    
    page_title = c.nombre
    args ={'form':form,'c':c,'prodescat':prodescat,}
