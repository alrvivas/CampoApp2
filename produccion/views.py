...
def producciones_esperadas(request):
    ...
    proescat = ProduccionEsperadaCategoria.objects.filter(fecha_a_agendar=datetime.datetime.today) 
    ...
    if query:
        ...
        resultspec = ProduccionEsperadaCategoria.objects.filter(qset)
        results = ProduccionEsperada.objects.filter(qset)
        template_name = "searchp.html"
        return render_to_response(template_name, {"resultspec":resultspec,"results": results,"query": query,'empleado':empleado},context_instance=RequestContext(request)) 
    else:
        results = []
    template ="esperadas.html"
    produccion_e  = ProduccionEsperada.objects.filter(fecha_a_agendar=datetime.datetime.today)    
    args ={'proescat':proescat,'produccion_e':produccion_e,'page_title':page_title,'empleado':empleado}
...
def produccion_realizada(request, producto_slug, template_name="produccion-realizada.html"):
    ...
    page_title      = p.name
    produccion_e 	= ProduccionEsperada.objects.filter(producto=p,fecha_a_agendar=datetime.datetime.today())
    produccion_rs   = ProduccionEsperada.objects.filter(producto=p,fecha_a_agendar__gt=datetime.datetime.today() - datetime.timedelta(days=7))
