...
from .forms import devolucionForm, devolucionMalaForm,devolucionReprocesoForm,devolucionMalaAlmacenForm,devolucionReprocesoAlmacenForm
...
@login_required(login_url='/login/')
def add_devolucion_mala_almacen(request, producto_slug, template_name="add-devolucion-mala-almacen.html"):
    user        = request.user
    empleado    = Empleado.objects.filter(user= user)
    p               = get_object_or_404(Product, slug=producto_slug)
    devolucion_m    = DevolucionMalaAlmacen.objects.all()
    page_title      = p.name
    #devolucion_b   = ProduccionEsperada.objects.filter(producto=p,fecha_a_agendar__lt=datetime.datetime.today()+ datetime.timedelta(days=1))
    if request.method == 'POST':
        form            = devolucionMalaAlmacenForm(request.POST)
        formproducto    = stockForm(request.POST , instance=p)
        if form.is_valid() and formproducto.is_valid():
            devolucion_m     = form.save(commit = False)
            p = formproducto.save(commit = False)
        ...
        form    = devolucionMalaAlmacenForm()
        formproducto = stockForm(request.POST , instance=p)
    args = {}
    args.update(csrf(request))    
    args ={'form':form,'p':p,'devolucion_m':devolucion_m,}
...
@login_required(login_url='/login/')
def devoluciones_malas_almacen(request):
    user        = request.user
    empleado    = Empleado.objects.filter(user= user)
    page_title = "Devoluciones Malas Almacén"
    devolucion_m  = DevolucionMalaAlmacen.objects.filter(fecha_de_salida=datetime.datetime.today)
    query = request.GET.get('q', '')
    if query:
        qset = (
            Q(fecha_de_salida__icontains=query) 
        )
        results = DevolucionMalaAlmacen.objects.filter(qset)
        template_name = "search.html"
        return render_to_response(template_name, {"results": results,"query": query,'empleado':empleado},context_instance=RequestContext(request)) 
    else:
        results = []  
    template ="devoluciones-malas-almacen.html"  
    args ={'devolucion_m':devolucion_m,'page_title':page_title,'empleado':empleado}
    return render_to_response(template, args,context_instance=RequestContext(request)) 

@login_required(login_url='/login/')
def devolucion_mala_almacen(request, devolucion_almacen_id):
    user        = request.user
    empleado    = Empleado.objects.filter(user= user)
    devolucion_m = get_object_or_404(DevolucionMalaAlmacen, id=devolucion_almacen_id)
    page_title = devolucion_m.producto.name
    if request.method == 'POST':
        form = devolucionMalaAlmacenForm(request.POST , instance=devolucion_m)
        if form.is_valid():
            devolucion_m = form.save(commit = False)
            devolucion_m.save()
            return redirect('devoluciones_malas_almacen')
    else:
        form = devolucionMalaAlmacenForm(request.POST , instance=devolucion_m)
        args = {}
        args.update(csrf(request))
        args ={'form':form,'devolucion_m':devolucion_m}
    template_name="devolucion-mala-almacen.html"
    return render_to_response(template_name, locals(),context_instance=RequestContext(request))

@login_required(login_url='/login/')
def add_devolucion_reproceso_almacen(request, producto_slug, template_name="add-devolucion-reproceso-almacen.html"):
    user        = request.user
    empleado    = Empleado.objects.filter(user= user)
    p               = get_object_or_404(Product, slug=producto_slug)
    devolucion_r    = DevolucionReprocesoAlmacen.objects.all()
    page_title      = p.name
    #devolucion_b   = ProduccionEsperada.objects.filter(producto=p,fecha_a_agendar__lt=datetime.datetime.today()+ datetime.timedelta(days=1))
    if request.method == 'POST':
        form            = devolucionReprocesoAlmacenForm(request.POST)
        formproducto    = stockForm(request.POST , instance=p)
        if form.is_valid() and formproducto.is_valid():
            devolucion_r     = form.save(commit = False)
            p = formproducto.save(commit = False)
        ...
        form    = devolucionReprocesoAlmacenForm()
        formproducto = stockForm(request.POST , instance=p)
    args = {}
    args.update(csrf(request))    
    args ={'form':form,'p':p,'devolucion_r':devolucion_r,}
...
@login_required(login_url='/login/')
def devoluciones_reproceso_almacen(request):
    user        = request.user
    empleado    = Empleado.objects.filter(user= user)
    page_title = "Devoluciones De Reproceso Almacén"
    devolucion_r  = DevolucionReprocesoAlmacen.objects.filter(fecha_de_entrada=datetime.datetime.today)
    query = request.GET.get('q', '')
    if query:
        qset = (
            Q(fecha_de_entrada__icontains=query) 
        )
        results = DevolucionReprocesoAlmacen.objects.filter(qset)
        template_name = "searchd.html"
        return render_to_response(template_name, {"results": results,"query": query,'empleado':empleado},context_instance=RequestContext(request)) 
    else:
        results = []
    template ="devoluciones-reproceso-almacen.html"
    args ={'devolucion_r':devolucion_r,'page_title':page_title,'empleado':empleado}
    return render_to_response(template, args,context_instance=RequestContext(request)) 

@login_required(login_url='/login/')
def devolucion_reproceso_almacen(request, devolucion_almacen_id):
    user        = request.user
    empleado    = Empleado.objects.filter(user= user)
    devolucion_r = get_object_or_404(DevolucionReprocesoAlmacen, id=devolucion_almacen_id)
    page_title = devolucion_r.producto.name
    if request.method == 'POST':
        form = devolucionReprocesoAlmacenForm(request.POST , instance=devolucion_r)
        if form.is_valid():
            devolucion_r = form.save(commit = False)
            devolucion_r.save()
            return redirect('devoluciones_reproceso_almacen')
    else:
        form = devolucionMalaAlmacenForm(request.POST , instance=devolucion_r)
        args = {}
        args.update(csrf(request))
        args ={'form':form,'devolucion_r':devolucion_r}
    template_name="devolucion-reproceso-almacen.html"
    return render_to_response(template_name, locals(),context_instance=RequestContext(request))  
